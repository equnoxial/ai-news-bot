import os
     ...
     def get_ai_post(title: str) -> Optional[str]:
         ...
         prompt = (
             f"Новость: {title}\n"
             "Напиши пост для Telegram на русском: заголовок + 4-6 коротких простых предложений + 1 риторический
     вопрос в конце в отдельном абзаце. "
             "Без ссылок, можно немного эмодзи в начале 1-го предложения, без хэштегов, используй абзацы, выделения
     ключевых слов для телеграмм канала."
         )
     ...
     def fetch_newest_entry(posted_links: set):
         """Собирает записи из всех фидов, возвращает самую свежую неопубликованную."""
         best_entry = None
         best_time = 0
